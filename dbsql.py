...
SQL_CONFIG = {
    ...
    'server': 'LOCALHOST',  # Thay bằng IP Server Production
    'database': 'QC_DASHBOARD_DB',
    'user': 'sa',           # Thay bằng user thật
    'password': 'YourStrongPassword', # Thay bằng pass thật
...
def save_batch_coils_v2(batch_data):
    """
    Lưu Batch Cuộn (Sử dụng MERGE thay cho ON CONFLICT của SQLite)
    """
    conn = get_connection()
    cursor = conn.cursor()
    try:
        # Sử dụng MERGE statement của T-SQL để UPSERT
        # Lưu ý: raw_data và scores là JSON string
        merge_query = """
        MERGE coil_data AS target
        USING (VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)) 
            AS source (coil_id, grade, raw_data, scores, is_checked, weight, target_thick, target_width, production_date, slab_grade)
        ...
        WHEN MATCHED THEN
            UPDATE SET 
                ...
                -- Không update is_checked để tránh mất trạng thái đã check
        WHEN NOT MATCHED THEN
            INSERT (coil_id, grade, raw_data, scores, is_checked, weight, target_thick, target_width, production_date, slab_grade)
            VALUES (source.coil_id, source.grade, source.raw_data, source.scores, source.is_checked, source.weight, source.target_thick, source.target_width, source.production_date, source.slab_grade);
        """
        ...
        # pyodbc hỗ trợ executemany với fast_executemany=True (Cần bật nếu data lớn)
